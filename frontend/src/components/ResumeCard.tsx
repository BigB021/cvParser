...
export const ResumeCard: React.FC<ResumeCardProps> = ({ resume, onDelete }) => {
  ...
  return (
    <div className="group bg-white border border-gray-200 rounded-2xl p-6 my-4 shadow-lg hover:shadow-2xl transition-all duration-300 max-w-xl mx-auto sm:mx-0 hover:-translate-y-1 relative overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          <InfoCard icon={<MapPin size={20} />} label="Location" value={city} />
          <InfoCard icon={<Clock size={20} />} label="Experience" value={`${exp_years} ${exp_years === 1 ? 'year' : 'years'}`} />
        </div>
          ...
          <div className="mb-6">
            ...
            <div className="space-y-2">
              {degrees.map((degree, index) => (
                <div
                  key={index}
                  className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-lg p-3 border-l-4 border-indigo-400"
                >
                  <span className="text-gray-800 font-medium">
                    {degree.degree_type}
                    {degree.degree_subject ? ` in ${degree.degree_subject}` : ""}
                  </span>
                </div>
              ))}
            </div>
...
const ContactDetail: React.FC<{ icon: React.ReactNode; value: React.ReactNode }> = ({ icon, value }) => (
  <div className="flex items-center">
    <div className="text-gray-600 mr-3">{icon}</div>
